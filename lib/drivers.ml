...
module Cli = struct
  ...
  module Terms = struct
    ...
    let auth_token =
      let doc =
        "Some operations require authenticating with adventofcode.com . This is the \
    ...
    let submit =
      let doc = "If set, attempts to submit the problem output to adventofcode.com." in
          ...
          | token, false ->
            Ok
              (Problem_runner.Run_mode.Test_from_puzzle_input
                 { credentials =
                     Option.map ~f:Problem_runner.Credentials.of_auth_token token
                 })
          | Some token, true ->
            Ok
